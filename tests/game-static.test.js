...
test('menu modes and model display behavior are wired', () => {
  ...
  assert.match(ui, /makeButton\(390, "Train mode — locked", FLOWCLIMB_MODES\.TRAIN, \{ enabled: false \}\)/)
  assert.match(ui, /makeButton\(470, "Flow mode", FLOWCLIMB_MODES\.FLOW\)/)
  ...
  assert.match(ui, /showFlowModeIntro\(\)/)
  assert.match(ui, /Train mode is locked\. Flow mode uses the active adaptive difficulty model\./)
  assert.match(ui, /Train mode collects gameplay data for the FlowClimb experiment\./)
  assert.match(ui, /Flow mode uses an adaptive difficulty model\. The game will adjust as you play\./)
  assert.match(ui, /beneficial if you play a couple of games, preferably with some rest in-between sessions/)
...
test('mobile teleport button appears when unstuck is available', () => {
  ...
  assert.match(runStateSource(), /Teleport available\\nPress U or tap Teleport/)
  assert.match(gameSource(), /wordWrap: \{ width: SCREEN_WIDTH - 72 \}/)
...
test('flow ML model loads active ONNX model with blocking failure behavior', () => {
  ...
  assert.match(gameRulesSource(), /window\.setInterval\(\(\) => \{/)
  assert.match(gameRulesSource(), /"\."\.repeat\(dotCount\)/)
  assert.match(gameRulesSource(), /window\.clearInterval\(loadingAnimation\)/)
